...
class CrawlController extends CrudController
{
        ...
        try {
            ...
            foreach ($request['link'] as $link) {
                if (preg_match('/(.*?)(\/phim\/)(.*?)/', $link)) {
                    $link = sprintf('%s/film/%s', config('kkphim_crawler.domain', 'https://phim.nguonc.com/api'), explode('film/', $link)[1]);
    ...
    public function showCrawlPage(Request $request)
    {
        ...
        try {
            $categories = Cache::remember('kkphim_categories', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/the-loai', config('kkphim_crawler.domain', 'https://phim.nguonc.com/api'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });

            $regions = Cache::remember('kkphim_regions', 86400, function () {
                $data = json_decode(file_get_contents(sprintf('%s/quoc-gia', config('kkphim_crawler.domain', 'https://phim.nguonc.com/api'))), true) ?? [];
                return collect($data)->pluck('name', 'name')->toArray();
            });
        } catch (\Throwable $th) {
    ...
    {
        $pattern = sprintf('%s/film/{slug}', config('kkphim_crawler.domain', 'https://phim.nguonc.com/api'));
        try {
